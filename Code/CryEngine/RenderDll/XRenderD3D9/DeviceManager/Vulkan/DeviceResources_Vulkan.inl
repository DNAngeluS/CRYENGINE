...
#pragma clang diagnostic pop
		...
		const VkImageViewType viewType =
				m_eTT == eTT_1D ? (numSlices == 1 ? VK_IMAGE_VIEW_TYPE_1D : VK_IMAGE_VIEW_TYPE_1D_ARRAY) : m_eTT == eTT_2D ? (numSlices == 1 ? VK_IMAGE_VIEW_TYPE_2D : VK_IMAGE_VIEW_TYPE_2D_ARRAY) : m_eTT == eTT_2DArray ? (numSlices == 1 ? VK_IMAGE_VIEW_TYPE_2D : VK_IMAGE_VIEW_TYPE_2D_ARRAY) : m_eTT == eTT_Cube ? (numSlices <= 6 ? VK_IMAGE_VIEW_TYPE_CUBE : VK_IMAGE_VIEW_TYPE_CUBE_ARRAY) : m_eTT == eTT_CubeArray ? (numSlices <= 6 ? VK_IMAGE_VIEW_TYPE_CUBE : VK_IMAGE_VIEW_TYPE_CUBE_ARRAY) : VK_IMAGE_VIEW_TYPE_3D;
		...
		case DXGI_FORMAT_X32_TYPELESS_G8X24_UINT:
			if (pImage->GetFormat() == VK_FORMAT_S8_UINT || pImage->GetFormat() == VK_FORMAT_D16_UNORM_S8_UINT || pImage->GetFormat() == VK_FORMAT_D24_UNORM_S8_UINT || pImage->GetFormat() == VK_FORMAT_D32_SFLOAT_S8_UINT)
			{
...
SResourceMemoryAlignment CDeviceTexture::GetAlignment(uint8 mip /*= 0*/, uint8 slices /*= 0*/) const
{
	...
	if (!(Layout.m_nWidth = Layout.m_nWidth >> mip))
		Layout.m_nWidth = 1;
	if (!(Layout.m_nHeight = Layout.m_nHeight >> mip))
		Layout.m_nHeight = 1;
	if (!(Layout.m_nDepth = Layout.m_nDepth >> mip))
		Layout.m_nDepth = 1;
