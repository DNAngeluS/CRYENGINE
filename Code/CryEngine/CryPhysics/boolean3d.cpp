...
{
	int i;
	float t, e[4], estart[4] = {0, (pVtx[ivtx0 + 1].pt - pVtx[ivtx0].pt).len2(), (pVtx[ivtx0 + 2].pt - pVtx[ivtx0 + 1].pt).len2(), (pVtx[ivtx0].pt - pVtx[ivtx0 + 2].pt).len2()};
  ...
  assert(0);
		...
		for (i = 0; i < nPolies; i++)
		{
			...
			for (dstflags = 0, ncont = 1;; dstflags += contour_end)
			{
				...
				ivtx = pPolies[i].ivtx;
				do
				{ // always start from a point on triangle boundary, if it exists
							...
							iszero(ipt2d[pResTris[icont * 3 + 1] + 1] - ipt2d[pResTris[icont * 3 + 2] + 1])))
				{
					...
					if (pRes)
					{
						...
						for (j = 0; j < 3; j++)
						{
							...
							for (ipt = 0; ipt < 3; ipt++)
								pRes->pNewTri[nNewTris].area[j][ipt] = (1 - iop * 2) * (pPolies[i].n * (pVtx[pPolies[i].ivtx0 + 2 - inc_mod3[ipt]].pt - m_pVertices[m_pIndices[itri * 3 + j]] ^
...
if (!IsHeapValid())
  assert(0);
		...
		m_nIslands = 0;
		for (i = 1; i < CRY_ARRAY_COUNT(m_bConvex); i++)
