...
CArticulatedEntity::CArticulatedEntity(CPhysicalWorld *pWorld, IGeneralMemoryHeap *pHeap)
		: CRigidEntity(pWorld, pHeap), m_infos(nullptr), m_joints(nullptr), m_nJoints(0), m_nJointsAlloc(0), m_posPivot(ZERO), m_offsPivot(ZERO), m_acc(ZERO), m_wacc(ZERO), m_simTime(0.0f), m_simTimeAux(10.0f), m_scaleBounceResponse(1.0f), m_bGrounded(0), m_nRoots(0), m_bInheritVel(0), m_pHost(nullptr), m_posHostPivot(ZERO), m_qHostPivot(IDENTITY), m_velHost(ZERO), m_bCheckCollisions(0), m_bFeatherstone(1), m_bExertImpulse(0), m_iSimType(1), m_iSimTypeLyingMode(1), m_iSimTypeCur(1), m_iSimTypeOverride(0), m_bIaReady(0), m_bPartPosForced(0), m_bFastLimbs(0), m_maxPenetrationCur(0.0f), m_bUsingUnproj(0), m_bUpdateBodies(1), m_nDynContacts(0), m_bInGroup(0), m_bIgnoreCommands(0), m_Ejoints(0.0f), m_nCollLyingMode(5), m_gravityLyingMode(m_gravity), m_dampingLyingMode(0.2f), m_EminLyingMode(sqr(0.12f)), m_nBodyContacts(0), m_rhistTime(0.0f), m_lockJoints(0), m_pCollEntList(nullptr), m_nCollEnts(0), m_bContactsAssigned(0)
{
...
CArticulatedEntity::~CArticulatedEntity()
{
	...
	if (m_infos)
		delete[] m_infos;
	if (m_pSrc && m_pWorld)
		m_pWorld->DestroyPhysicalEntity(m_pSrc, 4);
}
...
void CArticulatedEntity::AlertNeighbourhoodND(int mode)
{
	if (m_pHost)
	{
		m_pHost->RemoveCollider(this);
		--m_pHost->m_nSyncColliders;
		m_pHost->Release();
	}
	m_pHost = 0;
...
int CArticulatedEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams *_params, int id, int bThreadSafe)
{
	...
	if (m_joints[i].iStartPart + m_joints[i].nParts < idx)
	{
		...
		for (j = 0; j < NMASKBITS && getmask(j) <= m_constraintMask; j++)
			if (m_constraintMask & getmask(j))
				for (int iop = 0; iop < 2; iop++)
					if (m_pConstraints[j].pent[iop] == this && m_pConstraints[j].ipart[iop] >= idx)
...
void CArticulatedEntity::RerootJoint(int idx, int iParent, int flagsParent, const Ang3 &qextParent)
{
	...
	pj.qext = Ang3(ZERO);
	m_joints[idx1].body.q.Normalize();
	m_joints[idx1].quat.Normalize();
	...
	if (iParent < 0)
	{
		...
		pj.limits[0] = Vec3(-1e+10f);
		pj.limits[1] = Vec3(1e10f);
	}
	else if (iParent == m_pSrc->m_joints[idx].iParent)
	{
		...
		for (int iop = 0; iop < 2; iop++)
			pj.limits[iop] = m_pSrc->m_joints[idx].limits[iop];
	}
	else
	{
		idx0 = iParent;
		...
		for (int i = 0; i < 3; i++)
		{
			Vec3 ort1 = pj.q0 * ort[i];
			axsg[i] = -sgnnz(ort1[axmap[i] = idxmax3(ort1.abs())]);
		}
		for (int i = 0; i < 3; i++)
			pj.flags |= (flagsLock >> i & angle0_locked) << axmap[i] | (flagsLimit >> i & angle0_limit_reached) << axmap[i];
...
bool CArticulatedEntity::Reroot(int inewRoot)
{
	...
	if (m_pSrc->m_joints[inewRoot].idbody != m_joints[0].idbody)
	{
		for (int i = 0; i < m_nJoints; i++)
		{
			...
			m_joints[i].iParent = -2;
			m_joints[i].nChildren = m_joints[i].nChildrenTree = 0;
		}
		int inewRoot1 = m_infos[m_pSrc->m_joints[inewRoot].iStartPart].iJoint;
		m_offsPivot = (m_posPivot = m_joints[inewRoot1].body.pos) - m_pos;
		if (m_bGrounded)
		{
			m_body.v = m_joints[inewRoot1].body.v;
			m_body.w = m_joints[inewRoot1].body.w;
		}
		m_bGrounded = 1;
		RerootJoint(inewRoot, -1, 0, Ang3(ZERO));
		for (int i = 0; i < NMASKBITS && getmask(i) <= m_constraintMask; i++)
			if (m_constraintMask & getmask(i))
				for (int j = 0; j < 2; j++)
					if (m_pConstraints[i].pent[j] == this)
...
static inline Quat mirror_flipy(const Quat &src, const Vec3 &n)
{
	...
	for (int iax = 0; iax < 3; iax++)
		srcM.SetColumn(iax, mirror(const_cast<Quat &>(src).GetColumn(iax) * (1 - (iax & 1) * 2), n));
	return Quat(srcM);
}
...
void CArticulatedEntity::MirrorInto(int jntSrc, int jntDst, CArticulatedEntity *pdst)
{
	...
	Vec3 dir(ZERO);
	dir[idxmax3((jnt[j2src(jntSrc)].body.pos - jnt[j2src(jntDst)].body.pos).abs())] = 1;
	for (int i = 0, j; i < m_nJoints; i++)
		if (jnt[i].bHasExtContacts < 0)
		{
			for (j = (i ? i + 1 : m_nJoints); j < m_nJoints; j++)
				if (jnt[jnt[i].iParent].bHasExtContacts == jnt[j].iParent)
				{
					float score = 1;
					if (jnt[jnt[i].iParent].nChildren != 1 || jnt[jnt[j].iParent].nChildren != 1)
					{
						...
						else if ((tcode >> 4) == (tcode & 15))
						{
							...
							for (int iax = 0; iax < 3; iax++)
								bbox[0].Basis.SetRow(iax, mirror(bbox[0].Basis.GetRow(iax), dir));
							Vec3 sz1 = (bbox[0].Basis * (bbox[1].size * bbox[1].Basis)).abs();
							for (int iop = 0; iop < 3; iop++)
								score *= min(bbox[0].size[iop], sz1[iop]) / max(bbox[0].size[iop], sz1[iop]);
						}
						else
							score = 0;
					}
	...
	for (int i = 0; i < m_nJoints; i++)
	{
		...
		for (int ivel = 0; ivel < 4; ivel++)
			(&jdst.body.P)[ivel] = mirror((&jsrc.body.P)[ivel], dir);
		pdst->SyncJointWithBody(i, 1);
		pdst->SyncBodyWithJoint(i, 1);
	}
...
int CArticulatedEntity::SetParams(pe_params *_params, int bThreadSafe)
{
	...
	if (CRigidEntity::SetParams(_params, 1))
	{
		...
		if (_params->type == pe_params_part::type_id)
		{
			...
			if (i < m_nParts)
			{
				...
				if (!is_unused(params->pos) && !is_unused(params->invTimeStep) && m_pHost && i == i0)
				{ // Calculate the joint's v,w based on position+orientation changes
	...
	if (_params->type == pe_params_joint::type_id)
	{
		...
		{
			for (i = 0, m_joints[op[1]].selfCollMask = 0; i < m_nParts; i++)
				for (j = 0; j < params->nSelfCollidingParts; j++)
					if (m_parts[i].id == params->pSelfCollidingParts[j])
						m_joints[op[1]].selfCollMask |= getmask(i);
		}

		float rdt = !is_unused(params->ranimationTimeStep) ? params->ranimationTimeStep : (!is_unused(params->animationTimeStep) ? 1.0f / params->animationTimeStep : 0);
		for (i = 0; i < 3; i++)
		{
			...
			if (!is_unused(params->qext[i]) && m_joints[op[1]].iParent == op[0])
			{
				...
				if (!(m_joints[op[1]].flags & angle0_locked << i) &&
						isneg(m_joints[op[1]].limits[0][i] - m_joints[op[1]].qext[i]) + isneg(m_joints[op[1]].qext[i] - m_joints[op[1]].limits[1][i]) +
										isneg(m_joints[op[1]].limits[1][i] - m_joints[op[1]].limits[0][i]) <
								2)
	...
	if (_params->type == pe_params_articulated_body::type_id)
	{
		...
		if (!is_unused(params->bCollisionResp))
			m_bFeatherstone = !params->bCollisionResp;
		if (!is_unused(params->bFeatherstone))
			if (m_bFeatherstone = params->bFeatherstone)
				m_iSimType = m_iSimTypeLyingMode = 0;
	...
	if (_status->type == pe_status_dynamics::type_id)
	{
		...
		if ((unsigned int)i >= (unsigned int)m_nParts)
		{
			...
			if (m_body.M > 0)
			{
				...
				if (status->mass)
					status->centerOfMass *= __fres(status->mass);
			}
			else
...
int CArticulatedEntity::Action(pe_action *_action, int bThreadSafe)
{
	...
	if (_action->type == pe_action_impulse::type_id)
	{
		...
		int i, j;
		if (!m_bFeatherstone && action->impulse.len2() < sqr(m_body.M * 0.04f) && action->iSource == 0 && !(m_flags & aef_recorded_physics) || action->iSource == 4 || m_nJoints <= 0)
			return 1;
	...
	if (_action->type == pe_action_resolve_constraints::type_id)
	{
		...
		m_joints[0].body.v.zero();
		m_joints[0].body.w.zero();
		m_body.v.zero();
		masktype constrMask = MaskIgnoredColliders(iCaller);
		for (i = 0, m_constrInfoFlags = 0; i < NMASKBITS && getmask(i) <= m_constraintMask; i++)
			if (m_constraintMask & getmask(i))
		...
		for (int iter = 0;; iter++)
		{
			...
			for (int j = i = 0; j < m_nCollEnts; j++)
				if (!IgnoreCollision(m_pCollEntList[j]->m_collisionClass, m_collisionClass))
					m_pCollEntList[i++] = m_pCollEntList[j];
			m_nCollEnts = i;

			float bounceVel = m_pWorld->m_vars.minBounceSpeed;
			m_pWorld->m_vars.minBounceSpeed = 1e10f;
		retry_step:
			...
			for (i = 0; i < ncont; i++)
				if ((pcont[i]->flags & (contact_constraint_3dof | contact_angular)) == (contact_constraint_3dof | contact_angular) && pcont[i]->pent[1]->m_id < 0)
			...
			ncont = i;
			float dtback = 0;
			for (i = nconstr = ncoll = 0; i < ncont; i++)
				if (!(pcont[i]->flags & (contact_angular | contact_constraint)))
				{
					nconstr += pcont[i]->bProcessed = 1;
					ncoll++;
					...
					switch (pcont[i]->flags & contact_constraint)
					{
					case contact_constraint_3dof:
						nconstr += pcont[i]->bProcessed = 3;
						break;
					case contact_constraint_2dof:
						nconstr += pcont[i]->bProcessed = 1;
						break;
					case contact_constraint_1dof:
						nconstr += pcont[i]->bProcessed = 2;
						break;
					}
			if (dtback > 0)
			{
			...
			if (iter >= action->maxIters || min(nconstr, narg) == 0)
				break;
			float maxDrift = 0;
			for (int j = (i = -1) + 1; j < ncont; j++)
				if (!(pcont[j]->flags & contact_angular) && pcont[j]->vreq.len2() > maxDrift)
					maxDrift = pcont[i = j]->vreq.len2();
			if (i < 0 || !pcont[i]->bConstraint)
				break;
			float rdt = m_pConstraintInfos[pcont[i]->iConstraint - 1].hardness;
			dt = 1 / rdt;
			if (maxDrift * sqr(dt) > sqr(action->lastDist))
			{
				dt *= action->stepHardness;
				rdt *= invHardness; // time step to resolve the maximum constraint error * stepHardness
			}
			else
			...
			memset(H, 0, sqr(narg) * sizeof(H[0]));
			memset(A, 0, narg * nconstr * sizeof(A[0]));
			memset(arg, 0, narg * sizeof(arg[0]));

			for (int ic = i = 0; ic < ncont; ic++)
				if (!(pcont[ic]->flags & (contact_angular | contact_constraint)))
				{
					lbA[i] = pcont[ic]->vreq * pcont[ic]->n;
					ubA[i] = 100;
					i++; // collision contact; enforce non-penetration along n
				}
				else
					switch (pcont[ic]->flags & contact_constraint)
					{
					case contact_constraint_3dof:
					{ // full constaint; limit all 3 directions
						Vec3qp vreq = pcont[ic]->vreq, vleeway = pcont[ic]->vreq.abs() * 0.05f + Vec3(e);
						*(Vec3qp *)(lbA + i) = vreq - vleeway;
						*(Vec3qp *)(ubA + i) = vreq + vleeway;
						i += 3;
					}
					break;
					case contact_constraint_2dof:
						lbA[i] = (ubA[i] = pcont[ic]->vreq * pcont[ic]->n + e) - e * 2;
						break; // 2 dof constraint; lock rotation around n
					case contact_constraint_1dof:
					{ // 1 dof constraint; choose 2 axes orthogonal to n and limit rotation around them
			...
			for (i = 0; i < (1 - m_bGrounded) * 3; i++)
			{ // if not grounded, the first 3 arguments are the pivot's linear velocity
				for (int ic = 0, ici = 0; ic < ncont; ici += pcont[ic++]->bProcessed)
				{
					if (!(pcont[ic]->flags & (contact_angular | contact_constraint)))
						A[ici * narg + i] = pcont[ic]->n[i];
					else if (!(pcont[ic]->flags & contact_angular))
						switch (pcont[ic]->flags & contact_constraint)
						{
						case contact_constraint_3dof:
							A[(ici + i) * narg + i] = 1;
			...
			for (int j = 0; j < m_nJoints; j++)
			{
				...
				for (int ang = 0; ang < 3; ang++)
					if (!(m_joints[j].flags & angle0_locked << ang))
					{
						...
						{
							float E = 0;
							for (int c = 0; c <= m_joints[j].nChildrenTree; c++)
...
#define ChainToRoot(idx)                                               \
	for (int j##idx = j; j##idx >= 0; j##idx = m_joints[j##idx].iParent) \
		for (int ang##idx = 0; ang##idx < 3; ang##idx++)                   \
			if (!(m_joints[j##idx].flags & angle0_locked << ang##idx))       \
			{                                                                \
	...
	{
		for (int j = 0; j < ndriftable; j++)
			lb[j] -= edrift, ub[j] += edrift;
	}
	if (ndrifts < 0)
		break;
...
if (ncoll)
{
	EventPhysSimFinished epsf;
	InitEventBase(&epsf, this);
	epsf.frameData = nullptr;
	epsf.time = 0;
	epsf.numColl = ncoll;
...
int CArticulatedEntity::Step(float time_interval)
{
	...
	if (m_bCheckCollisions)
	{
		...
		for (i = 0; i < m_nCollEnts; i++)
			if (m_pCollEntList[i]->m_iSimClass > 2 && !IgnoreCollision(m_pCollEntList[i]->m_collisionClass, m_collisionClass))
		...
		if (m_iSimClass <= 2)
		{
			for (i = j = 0; i < m_nCollEnts; i++)
				if (!IgnoreCollision(m_pCollEntList[i]->m_collisionClass, m_collisionClass))
			...
			for (j = 0; j < 3; j++)
				if (!(m_joints[i].flags & angle0_locked << j) &&
						isneg(m_joints[i].limits[0][j] - m_joints[i].qext[j]) + isneg(m_joints[i].qext[j] - m_joints[i].limits[1][j]) +
										isneg(m_joints[i].limits[1][j] - m_joints[i].limits[0][j]) <
								2)
				{ // qext violates limits; adjust the limits
					float diff[2];
...
int CArticulatedEntity::CalcBodyIa(int idx, matrixf &Ia_change, int bIncludeLimits)
{
	...
	for (i = 0; i < 3; i++)
		if ((m_joints[idx].flags & (angle0_locked | angle0_limit_reached) << i & -bIncludeLimits) == angle0_limit_reached << i)
		{
			m_joints[idx].fs->qidx2axidx[i] = j;
			m_joints[idx].fs->axidx2qidx[j++] = i;
		}
...
int CArticulatedEntity::StepJoint(int idx, float time_interval, int &bBounced, int bFlying, int iCaller)
{
	...
	else
	{
		...
		if (m_constrInfoFlags & constraint_instant)
			for (i = 0; i < NMASKBITS && getmask(i) <= m_constraintMask; i++)
				...
				{ // for plane-align (2 dof locked) angular constraints support instant positional enforcement
					...
					Vec3 proj;
					for (j = 0; j < 3; j++)
						proj[j] = fabs(m_joints[idx].rotaxes[j] * (vcur ^ vdst));
					int idxsort[3];
					idxsort[1] = 3 - (idxsort[0] = idxmax3(proj)) - (idxsort[2] = idxmin3(proj));
					for (int j1 = 0; j1 < 3; j1++)
						if (!(m_joints[idx].flags & angle0_locked << (j = idxsort[j1])))
						{
	...
	{ // && m_joints[idx].bAwake) {
		...
		if (itmax >= 0)
		{
			if (pcontacts[itmax].iUnprojMode)
			{
				...
				{ // note: update of parent will possibly invalidate its registered contacts, is it crucial?
...
void CArticulatedEntity::DrawHelperInformation(IPhysRenderer *pRenderer, int flags)
{
	...
	{
		for (int i = 0; i < m_nJoints; i++)
			if ((m_joints[i].idbody | idsel >> 31) == idsel)
			{
				quaternionf q_parent;
				if (m_joints[i].iParent >= 0)
...
void PropagateImpulses(strided_pointer<ArticulatedBody> joints, const Vec3 &dv)
{
	for (int idx = 0, iparent; idx <= joints[0].nChildrenTree; idx++)
	{
		...
		{
			joints[idx].fs->dv_vec[0].zero();
			joints[idx].fs->dv_vec[1] = dv;
		}
		...
		joints[idx].fs->Ya_vec[0].zero();
		joints[idx].fs->Ya_vec[1].zero();
	}
}
...
void ArticulatedBody::GetContactMatrix(const Vec3 &r, Matrix33 &K)
{
	...
	if (dirsing * (K * dirsing) < dirsing.len2() * body.Minv * 0.001f)
	{ // check if the direction towards pivot is singular for movement
...
void ArticulatedBody::GetContactMatrixRot(Matrix33 &K, ArticulatedBody *buddy)
{
	...
	else if (buddy && &joints1[buddy->fs->iparent] == this)
	{ // buddy is our child
...
int ArticulatedBody::ApplyImpulse(const Vec3 &dP, const Vec3 &dL, body_helper *bodies, int iCaller)
{
	...
	if (fs->useTree < 0)
	{
		for (; joints[0].fs->iparent; joints = joints + joints[0].fs->iparent)
			;
		int useTree = 0;
		for (int i = 0; i <= joints[0].nChildrenTree; i++)
			if (joints[i].body.bProcessed[iCaller])
	...
	if (!fs->useTree)
	{
		...
		bodies[ibody].v = body.v;
		bodies[ibody].w = body.w;
		bodies[ibody].L += dL;
	}
	else
	{
		...
		for (int i = 0, j; i <= joints[0].nChildrenTree; i++)
			if (j = joints[i].body.bProcessed[iCaller])
			{
	...
	if (!m_bContactsAssigned)
	{
		...
		for (entity_contact *pContact = m_pContactStart; pContact != CONTACT_END(m_pContactStart); pContact = pContact->next)
			if (!(pContact->flags & contact_2b_verified))
			{
				...
				pContact->nextAux = m_joints[j].pContact;
				m_joints[j].pContact = pContact;
			}
	...
	for (idx = 0; idx < m_nJoints; idx++)
	{
		...
		if (!m_bFeatherstone && (m_bGrounded || m_joints[idx].iParent >= 0))
		{
			...
			for (i = j = 0, ks = kd = 0; i < 3; i++)
				if (!(m_joints[idx].flags & angle0_locked << i))
				{
					iAxes[j++] = i;
...
int CArticulatedEntity::Update(float time_interval, float damping)
{
	if (m_bFeatherstone && m_nJoints > 0)
	{
		...
		if (m_joints[0].fs)
		{
			for (int i = 0; i < m_nJoints; i++)
			{
				m_joints[i].Pext *= m_scaleBounceResponse;
				m_joints[i].Lext *= m_scaleBounceResponse;
			}
