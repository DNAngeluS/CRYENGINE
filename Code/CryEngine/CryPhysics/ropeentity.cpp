...
CRopeEntity::CRopeEntity(CPhysicalWorld *pWorld, IGeneralMemoryHeap *pHeap)
		: CPhysicalEntity(pWorld, pHeap), m_gravity(ZERO), m_gravity0(ZERO), m_damping(0.2f), m_maxAllowedStep(0.05f), m_Emin(sqr(0.04f)), m_timeStepPerformed(0.0f), m_timeStepFull(0.0f), m_nSlowFrames(0), m_lastTimeStep(0.0f), m_nSleepingNeighboursFrames(0), m_bHasContacts(0), m_bContactsRegistered(0), m_lockVtx(0), m_lockAwake(0), m_length(0.0f), m_nSegs(-1), m_segs(nullptr), m_timeLastActive(0.0f), m_bTargetPoseActive(0), m_stiffnessAnim(70.0f), m_lastposHost(ZERO), m_lastqHost(IDENTITY), m_mass(1.0f), m_collDist(0.01f), m_surface_idx(0), m_friction(0.2f), m_stiffness(10.0f), m_dampingAnim(0.0f), m_stiffnessDecayAnim(0.75f), m_wind(ZERO), m_wind0(ZERO), m_wind1(ZERO), m_airResistance(0.0f), m_windVariance(0.0f), m_windTimer(0.0f), m_waterResistance(5.0f), m_rdensity(1.0f / 500), m_jointLimit(0.0f), m_jointLimitDecay(0), m_szSensor(0.05f), m_maxForce(0.0f), m_flagsCollider(geom_colltype0), m_collTypes(0), m_penaltyScale(5.0f), m_maxIters(650), m_attachmentZone(0.0f), m_minSegLen(0.0f), m_unprojLimit(0.5f), m_noCollDist(0.5f), m_idConstraint(0), m_iConstraintClient(0), m_dir0dst(ZERO), m_vtx(nullptr), m_vtx1(nullptr), m_vtxSolver(nullptr), m_nVtx(0), m_nVtxAlloc(0), m_nVtx0(0), m_nFragments(0), m_nMaxSubVtx(3), m_idx(nullptr), m_bStrained(0), m_frictionPull(0.0f), m_energy(0.0f), m_pContact(nullptr), m_attach(nullptr), m_nAttach(0)
#ifdef DEBUG_ROPES
			,
...
{
	if (m_segs)
		delete[] m_segs; // cppcheck-suppress autovarInvalidDeallocation
	if (m_vtx)
		delete[] m_vtx;
	if (m_vtx1)
		delete[] m_vtx1;
		...
		if (m_segs)
			for (i = 0; i < 2; i++)
				if (!is_unused(params->pEntTiedTo[i]))
				{
					...
					m_pTiedTo[i] = params->pEntTiedTo[i] == WORLD_ENTITY ? &g_StaticPhysicalEntity : (params->pEntTiedTo[i] ? ((CPhysicalPlaceholder *)params->pEntTiedTo[i])->GetEntity() : 0);
	...
	if (_status->type == pe_status_rope::type_id)
	{
		...
		QuatT transG = is_unused(status->pGridRefEnt) ? QuatT(IDENTITY) : m_pWorld->GetGrid(status->pGridRefEnt)->m_transW.GetInverted() * m_pWorld->GetGrid(this)->m_transW;
		{
			...
			if (!is_unused(status->pPoints) && status->pPoints)
				for (i = 0; i <= m_nSegs; i++)
					status->pPoints[i] = transG * m_segs[i].pt0;
			if (!is_unused(status->pVelocities) && status->pVelocities)
				for (i = 0; i <= m_nSegs; i++)
					status->pVelocities[i] = transG.q * m_segs[i].vel;
...
void CRopeEntity::EnforceConstraints(float seglen, const quaternionf &qtv, const Vec3 &offstv, float scaletv, int bTargetPoseActive, float dt)
{
	...
	else if (m_nAttach || m_nSegs && (!(m_flags & rope_subdivide_segs) || m_maxIters < sqr(m_nSegs) * 10 && !m_bStrained && m_length > 0))
	{
		...
		for (i = 0; i < m_nAttach; m_segs[m_attach[i++].ivtx].kdP = 0)
			;
		for (i = 0; i < 2; i++)
			m_segs[m_nSegs * i].kdP = !m_pTiedTo[i];
		...
		do
		{
			...
			for (i = iStart, bTargetPoseActive = 0; i != iEnd; i += iDir)
			{
				...
				if ((1 - inrange(len2, seglen2 * 1.02f, seglen2 * 0.98f)) * m_segs[i + iDir].kdP)
				{
					...
					if (inrange(len2, seglen2 * 1.1f, seglen2 * 0.8f))
	...
	if (_action->type == pe_action_attach_points::type_id)
	{
		...
		else
		{
			...
			for (i = 0; i < action->nPoints; i++)
				if ((unsigned int)action->piVtx[i] > m_nSegs)
					return 0;
			Vec3 pos;
			Quat q;
			float scale;
	...
	if (_action->type == pe_action_slice::type_id)
	{
		...
		else
		{
			if (i < m_nSegs)
			{
				...
				{
					pr.nMaxSubVtx = 0;
					pentDst->m_flags |= rope_subdivide_segs;
				}
...
RecheckAfterStrain:
	...
	for (i = iStart; i <= iEnd; i++)
	{
		...
		for (j = 0; j < nCheckParts; j++)
			if (!checkParts[j].pGeom->IsAPrimitive())
			{ //checkParts[j].pent->m_iSimClass!=3) {
...
float CRopeEntity::Solver(float time_interval, float seglen)
{
	...
	if (m_bHasContacts + m_nAttach || m_flags & rope_subdivide_segs)
	{
		...
		if (!(m_flags & rope_subdivide_segs))
		{
			...
			for (i = 0; i < 2; i++)
				m_segs[i * m_nSegs].kdP = !m_pTiedTo[i];
			for (i = 0; i < m_nAttach; i++)
				m_segs[m_attach[i].ivtx].kdP = 0;
					...
					if (m_segs[i].pContactEnt)
					{
						...
						if ((vrel = dp * m_segs[i].ncontact) < (vreq = m_segs[i].vreq) - seglen * 0.005f)
						{
							if (m_friction > 0.01f)
							{
								...
								if (max(m_segs[i].dP, 1e-10f - fabs_tpl(vrel)) < 0)
		...
		else if (!m_bStrained || m_length == 0)
		{
			...
			for (i = 0; i < m_nVtx; m_vtx[i++].kdP = 1)
				;
			for (i = 0; i < 2; i++)
				m_vtx[i * (m_nVtx - 1)].kdP = !m_pTiedTo[i];
			for (i = 0; i < m_nAttach; i++)
				m_vtx[m_segs[m_attach[i].ivtx].iVtx0].kdP = 0;
			...
			for (j = 0, angle = 0; j < nCheckParts; j++)
			{
				...
				if (box_ray_overlap_check(&checkParts[j].bbox, &aray.m_ray))
				{
					...
					if (!bNoLineUnproj && checkParts[j].pGeom->Intersect(&aray, &gwd, 0, &ip, pcontact))
					{
						...
						{ // && (pcontact->pt-aray.m_ray.origin)*pcontact->n<0) {
...

		{
			...
			if (!partBVList || m_pOuterEntity != m_pWorld->m_threadData[iCaller].pTmpPartBVListOwner || !m_pOuterEntity || is_unused(m_collBBox[0]))
			{
				...
				for (i = 0, ippbv = 0; i < nEnts; i++)
					if (pentlist[i] != this && !IgnoreCollision(m_collisionClass, pentlist[i]->m_collisionClass) && !(m_flags & rope_ignore_attachments && (pentlist[i] == m_pTiedTo[0] || m_pTiedTo[0] && m_pTiedTo[0]->m_pForeignData && m_pTiedTo[0]->m_pForeignData == pentlist[i]->m_pForeignData || pentlist[i] == m_pTiedTo[1] || m_pTiedTo[1] && m_pTiedTo[1]->m_pForeignData && m_pTiedTo[1]->m_pForeignData == pentlist[i]->m_pForeignData)))
...
int CRopeEntity::RegisterContacts(float time_interval, int nMaxPlaneContacts)
{
	...
	for (iStart = j = 0; iStart < m_nVtx - 1; iStart = iEnd)
	{
		...
		iEnd = i;
		pContact->pbody[0] = pbody[0] = iStart == 0 ? (pContact->pent[0] = m_pTiedTo[0])->GetRigidBody(pContact->ipart[0] = m_iTiedPart[0], 1) : (pContact->pent[0] = m_vtx[iStart].pContactEnt)->GetRigidBody(pContact->ipart[0] = m_vtx[iStart].iContactPart, 1);
		pContact->pbody[1] = pbody[1] = iEnd == m_nVtx - 1 ? (pContact->pent[1] = m_pTiedTo[1])->GetRigidBody(pContact->ipart[1] = m_iTiedPart[1], 1) : (pContact->pent[1] = m_vtx[iEnd].pContactEnt)->GetRigidBody(pContact->ipart[1] = m_vtx[iEnd].iContactPart, 1);
		for (i = 0; i < 2; i++)
