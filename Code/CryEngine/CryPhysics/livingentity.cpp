...
CLivingEntity::CLivingEntity(CPhysicalWorld *pWorld, IGeneralMemoryHeap *pHeap)
		: CPhysicalEntity(pWorld, pHeap), m_vel(ZERO), m_velRequested(ZERO), m_gravity(0, 0, -9.81f), m_nslope(0.0f, 0.0f, 1.0f), m_dtRequested(0.0f), m_kInertia(8.0f), m_kInertiaAccel(0.0f), m_kAirControl(0.1f), m_kAirResistance(0.0f), m_hCyl(1.1f), m_hEye(1.7f), m_hPivot(0.0f), m_size(0.4f, 0.4f, 0.6f), m_dh(0.0f), m_dhSpeed(0.0f), m_dhAcc(0.0f), m_stablehTime(1.0f), m_hLatest(-1e-10f), m_nodSpeed(60.0f), m_mass(80), m_massinv(2.0f / 80), m_surface_idx(0), m_lastGroundSurfaceIdx(-1), m_lastGroundSurfaceIdxAux(-1), m_lastGroundPrim(-1), m_timeFlying(0.0f), m_timeForceInertia(0.0f), m_slopeSlide(cos_tpl(g_PI * 0.2f)), m_slopeClimb(cos_tpl(g_PI * 0.3f)), m_slopeJump(cos_tpl(g_PI * 0.3f)), m_slopeFall(cos_tpl(g_PI * 0.39f)), m_maxVelGround(10.0f), m_groundContactEps(GROUND_EPS_SIZE), m_timeImpulseRecover(5.0f), m_pCylinderGeom(nullptr), m_hHead(1.9f), m_pHeadGeom(nullptr), m_timeUseLowCap(-1.0f), m_timeSinceStanceChange(0.0f), m_timeSinceImpulseContact(10.0f), m_timeStepFull(0.0f), m_timeStepPerformed(0.0f), m_iSnapshot(0), m_iTimeLastSend(-1), m_collTypes(ent_terrain | ent_static | ent_sleeping_rigid | ent_rigid | ent_living), m_pLivingEntToIgnore(nullptr), m_bFlying(1), m_bJumpRequested(0), m_bSwimming(0), m_bUseCapsule(0), m_bIgnoreCommands(0), m_bStateReading(0), m_bActive(1), m_bActiveEnvironment(0), m_bStuck(0), m_bReleaseGroundColliderWhenNotActive(1), m_bHadCollisions(0), m_bSquashed(0), m_pLastGroundCollider(nullptr), m_iLastGroundColliderPart(0), m_posLastGroundColl(ZERO), m_velGround(ZERO), m_deltaPos(ZERO), m_posLocal(ZERO), m_deltaV(ZERO), m_deltaQRot(IDENTITY), m_timeSmooth(0.16f), m_timeRotChanged(-1.0f), m_pContacts(nullptr), m_nContacts(0), m_nContactsAlloc(0), m_pBody(nullptr), m_lockLiving(0), m_lockStep(0), m_forceFly(false)
{
	if (pWorld)
			...
			if ((heightCollider != hCyl || sizeCollider.x != size.x || sizeCollider.z != size.z) && m_pWorld && m_bActive)
			{
				do
				{
					unproj += (step = UnprojectionNeeded(pos + params->dirUnproj * unproj, qrot, heightCollider,
																							 hPivot, sizeCollider, m_bUseCapsule, params->dirUnproj, iCaller)) *
										1.01f;
				...
				m_pWorld && m_bActive)
		{
			do
			{
				unproj += (step = UnprojectionNeeded(m_pos + params->dirUnproj * unproj, m_qrot, params->heightCollider,
																						 params->heightPivot, params->sizeCollider, params->bUseCapsule, params->dirUnproj)) *
									1.01f;
...
int CLivingEntity::Step(float time_interval)
{
			 ...
			 m_bActiveEnvironment || m_nslope.z < m_slopeSlide || m_velGround.len2() > 0))
	{
		...
		{ //Rich S - is bFlyMode a cheat? Can we disable in release?
			pos += move;
			bFlying = 1;
			m_hLatest = 0;
			ReleaseGroundCollider();
		...
		else
		{
			...
			if (m_vel.len2() > sqr(0.01f) || m_velRequested.len2())
		...
		retry_without_ground_sync:
								...
								if (pPrecompParts[j1].partflags & flagsCollider && (!bCheckBBox || AABB_overlap(BBoxInner, pPrecompParts[j1].BBox)))
								{
									...
									if (icnt = pCurCyl->Intersect(pPrecompParts[j1].pgeom, gwd + igwd, gwd + 1, &ip, pcontacts))
									{
										...
										if (iCyl == 0 && bHeavy && !(pPrecompParts[j1].partflags & geom_no_coll_response))
										{
											...
											if (m_pWorld->m_bWorldStep == 2)
											{ // this means step induced by rigid bodies moving around
												// if the entity is rigid, store the contact
												int bPushOther;
												if (bPushOther = iSimClass > 0 && minv > 0)
												{ // && pent->m_iGroup==m_pWorld->m_iCurGroup) {
					...
					for (i = 0; i < nents; ++i)
					{
						...
						else
						{
							...
							if (kd >= 0)
							{
								...
								kd = sqrt_tpl(kd);
								float tfirst = -kb + kd;
								ncoll = Vec2(pos + gwd[0].v * tfirst - pent->m_pos);
			...
			if (m_pHeadGeom)
			{
				...
				for (i = 0; i < nents; i++)
					...
					{ //pentlist[i]->GetType()!=PE_LIVING && pentlist[i]->GetMassInv()*0.4f<m_massinv) {
...
void CLivingEntity::Step_HandleFlying(Vec3 &vel, const Vec3 &velGround, int bWasFlying, const Vec3 &heightAdj, const float kInertia, const float time_interval)
{
	...
	if (!m_pWorld->m_vars.bFlyMode)
	{
		...
		if ((gravity * heightAdj) < 0.f)
		{ // Remove any slope adjustment from the gravity accumulation
