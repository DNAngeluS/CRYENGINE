...
#ifdef MULTI_GRID
SEntityGrid *GetGridFromEntity(const CPhysicalEntity *pent)
{
	return pent->m_pWorld->GetGrid(pent);
}
...
CPhysicalWorld::CPhysicalWorld(ILog *pLog) : m_nWorkerThreads(0)
{
	...
	m_grpProfileData[13].pName = "of which the Solver";
	m_grpProfileData[13].nCallsLast = 1;
	m_grpProfileData[14].pName = "Queued requests";
	m_grpProfileData[14].nCallsLast = 1;
	m_grpProfileData[15].pName = "World step total";
	m_grpProfileData[15].nCallsLast = 1;
...
void SEntityGrid::Init()
{
	...
	rzGran = 16;
	iup = 2;
	pPODcells = &(pDummyPODcell = &dummyPODcell);
	dummyPODcell.lifeTime = 1E10f;
	dummyPODcell.zlim.set(1E10f, -1E10f);
	log2PODscale = 0;
	iActivePODCell0 = -1;
	bHasPODGrid = 0;
	dummyPODcell.zlim[0] = 1E10f;
	dummyPODcell.zlim[1] = 1E10f;
	m_host = nullptr;
...
void CPhysicalWorld::Shutdown(int bDeleteGeometries)
{
	...
	if (m_pRwiHitsHead)
	{
		...
		for (phit = (pchunk = m_pRwiHitsPool) + 1; phit->next != m_pRwiHitsPool + 1; phit = phit->next)
			if (!phit->next[-1].next)
			{ // means phit->next[-1] is a chunk header
...
IPhysicalEntity *CPhysicalWorld::SetupEntityGrid(int axisz, Vec3 org, int nx, int ny, float stepx, float stepy, int log2PODscale, int bCyclic, IPhysicalEntity *pHost, const QuatT &posInHost)
{
	...
	if (pHost && !(pgrid = GetHostedGrid((CPhysicalEntity *)pHost)))
	{
		...
		if (pgrid->m_next = m_entgrid.m_next)
			m_entgrid.m_next->m_prev = pgrid;
		pgrid->m_prev = &m_entgrid;
		m_entgrid.m_next = pgrid;
		pe_params_outer_entity poe;
		poe.pOuterEntity = pgrid;
		pHost->SetParams(&poe);
	...
	{
		int i;
		for (i = 0; i <= m_pEntsById.getMaxId(); i++)
			if (m_pEntsById[i] && GetGrid(m_pEntsById[i]) == pgrid)
			{
		...
		for (CPhysArea *pArea = m_pGlobalArea; pArea; pArea = pArea->m_next)
			if (GetGrid(pArea) == pgrid)
				DetachEntityGridThunks(pArea);
		for (i = pgrid->size.x * pgrid->size.y; i >= 0; i--)
			if (pgrid->cells[i])
...
int SEntityGrid::SetParams(pe_params *_params, int bThreadSafe)
{
	if (_params->type == pe_params_outer_entity::type_id)
	{
		...
		if (!is_unused(params->pOuterEntity) && params->pOuterEntity != (IPhysicalEntity *)m_host)
		{
			if (m_host)
				m_host->Release();
			if (m_host = (CPhysicalEntity *)params->pOuterEntity)
				m_host->AddRef();
			GetUpdatedTransW();
			GetUpdatedVelW();
		}
...
void SEntityGrid::DeactivateOnDemand()
{
	if (bHasPODGrid)
	{
		int i;
		pPODcells--;
		for (i = size.x * size.y >> (3 + log2PODscale) * 2; i >= 0; i--)
			if (pPODcells[i])
				delete[] pPODcells[i];
		delete[] pPODcells;
		pPODcells = &pDummyPODcell;
		dummyPODcell.lifeTime = 1E10f;
		iActivePODCell0 = -1;
		bHasPODGrid = 0;
	}
...
void SEntityGrid::RegisterBBoxInPODGrid(const Vec3 *BBox, IPhysicsStreamer *pStreamer)
{
	...
	Vec3 gBBox[2];
	BBoxToGrid(BBox[0], BBox[1], gBBox);
	for (i = 0; i < 2; i++)
	{
	...
	for (ix = igx[0]; ix <= igx[1]; ix++)
		for (iy = igy[0]; iy <= igy[1]; iy++)
		{
			...
			if (!pPODcells[i])
			{
				...
				for (int j = 0; j < 64; j++)
					pPODcells[i][j].zlim.set(1000.0f, -1000.0f);
				if (!imask)
					pPODcells[i][0].lifeTime = 1e10f;
			}
			...
			if (pPODcell->lifeTime > 0 && pPODcell->lifeTime < 1e10f)
			{
				...
				Vec3 center, sz;
				GetPODGridCellBBox(ix << log2PODscale, iy << log2PODscale, center, sz);
...
int SEntityGrid::AddRefEntInPODGrid(IPhysicalEntity *_pent, const Vec3 *BBox)
{
	...
	Vec3 gBBox[2];
	BBoxToGrid(pBBox[0], pBBox[1], gBBox);
	for (i = 0; i < 2; i++)
	{
																											...
																											int id, IPhysicalEntity *pHostPlaceholder, IGeneralMemoryHeap *pHeap)
{
	...
	case PE_WALKINGRIGID:
		res = CPhysicalEntity::Create<CWalkingRigidEntity>(this, pHeap);
		break;
	case PE_PARTICLE:
...
int CPhysicalWorld::DestroyPhysicalEntity(IPhysicalEntity *_pent, int mode, int bThreadSafe)
{
	...
	if (_pent->GetType() == PE_GRID)
	{
		...
		{
			if (pgrid->m_next)
				pgrid->m_next->m_prev = pgrid->m_prev;
			if (pgrid->m_prev)
				pgrid->m_prev->m_next = pgrid->m_next;
			if (pgrid == m_pDeletedGrids)
				m_pDeletedGrids = nullptr;
			pgrid->m_prev = nullptr;
			pgrid->m_next = m_pDeletedGrids;
			if (m_pDeletedGrids)
				m_pDeletedGrids->m_prev = pgrid;
...
#define prev m_next_coll1
	...
	for (pent = pent0; pent; pentLast = pent, pent = pent->next)
		pent->m_bProcessed_aux = 1 << 31;
	for (pent = pent0, pent1 = 0; pent; pent = pent->next)
		if (pent->m_pOuterEntity && pent->m_pOuterEntity->m_bProcessed_aux >= 0)
		{
			// temporarily append external outer ents to the list	(in m_bProcessed_aux mark bit 31 = in list, bit 30 = is external)
			pentLast->next = pent->m_pOuterEntity;
			pentLast = pent->m_pOuterEntity;
			pentLast->m_next_coll = pentLast->next; // store previous value of coll_next2
			pentLast->next = 0;
			pentLast->m_bProcessed_aux = 3 << 30;
		}
	for (pent = pent0, pent1 = 0; pent; pent->prev = pent1, pent1 = pent, pent = pent->next)
	...
	for (pent = pentLast = pent0; pent; pentLast = pent, pent = pent1)
	{
		...
		{
			if (pent->next)
				pent->next->prev = pent->prev;
			if (pent->prev)
				pent->prev->next = pent->next;
			pent->next = pent->m_pOuterEntity->next;
			pent->prev = pent->m_pOuterEntity;
			if (pent->m_pOuterEntity->next)
				pent->m_pOuterEntity->next->prev = pent;
																			...
																			CPhysicalEntity *pPetitioner, int szListPrealloc, int iCaller)
{
	...
	Vec3 gBBox[2];
	grid.BBoxToGrid(ptmin, ptmax, gBBox);
	float zdiff = gBBox[1].z - gBBox[0].z, zsum = gBBox[1].z + gBBox[0].z;
	for (int i = 0; i < 2; i++)
		gBBox[i] = gBBox[i] * Diag33(grid.stepr.x, grid.stepr.y, grid.rzGran);
	...
	IF((igx1 - igx0 + 1) * (igy1 - igy0 + 1) > m_vars.nGEBMaxCells, 0)
	{
		if (m_pLog)
			m_pLog->Log("GetEntitiesInBox: too many cells requested by %s (%d, (%.1f,%.1f,%.1f)-(%.1f,%.1f,%.1f))",
									pPetitioner && m_pRenderer ? m_pRenderer->GetForeignName(pPetitioner->m_pForeignData, pPetitioner->m_iForeignData, pPetitioner->m_iForeignFlags) : "Game",
							...
							else if (pGridEnt->m_iForeignFlags & itype)
							{
								...
								if (pGridEnt->m_iForeignFlags & TRIGGER_PORTAL && GetHostedGrid(pPetitioner) != GetGrid(pGridEnt->m_pEntBuddy))
								{
									...
									if (!pGridEnt->IsPlaceholder() && pentTrg->m_nParts)
									{
										...
										box bbox[2];
										bbox[0].Basis.SetIdentity();
										bbox[0].bOriented = 0;
										bbox[0].center = (ptmin + ptmax) * 0.5f;
										bbox[0].size = (ptmax - ptmin) * 0.5f;
									...
									Vec3 BBox1[2];
									transformBBox(ptmin, ptmax, BBox1, GetGrid(pGridEnt->m_pEntBuddy)->m_transW.GetInverted() * GetGrid(pGridEnt)->m_transW);
...
void SPhysTask::SignalStopWork()
{
	bStop = 1;
	m_pWorld->m_threadStart[m_idx].Set();
}
						...
						for (ipass = 0; ipass < 2; ipass++)
						{
							...
							for (pent = m_pTypedEnts[2], nGroups = 0, m_threadData[0].groupMass = 1.0f; pent; pent = pent_next)
							{
								...
								if (!(pent->m_bMoved | m_bUpdateOnlyFlagged & (pent->m_flags ^ pef_update) | bSkipFlagged & pent->m_flags))
								{
									...
									{
										if ((iter | ipass) == 0)
										{ // just make isolated step for rigids with infinite mass
											pent->StartStep(time_interval);
							...
							if (ipass == 0)
							{
								...
								m_pCurEnt = GroupByOuterEntity(m_pAuxStepEnt);
								m_pAuxStepEnt = 0;
...
int CPhysicalWorld::RepositionEntity(CPhysicalPlaceholder *pobj, int flags, Vec3 *BBox, int bQueued)
{
	...
	if (flags & 1 && pgrid->cells)
	{
		...
		Vec3 gBBox[2];
		pgrid->BBoxToGrid(pBBox[0], pBBox[1], gBBox);
		for (i = 0; i < 2; i++)
		{
			...
			if (igx[0] - pobj->m_ig[0].x | igy[0] - pobj->m_ig[0].y | igx[1] - pobj->m_ig[1].x | igy[1] - pobj->m_ig[1].y | flags & 4 | flags >> 2 & 1 ^ pobj->m_bOBBThunks)
			{
				...
				if (pobj->m_iSimClass != 5)
				{
					if (n == 0 || n > (unsigned int)m_vars.nMaxEntityCells)
					{
						...
						char buf[256];
						cry_sprintf(buf, "Error: %s @ %.1f,%.1f,%.1f is too large or invalid", !m_pRenderer ? "entity" : m_pRenderer->GetForeignName(pcurobj->m_pForeignData, pcurobj->m_iForeignData, pcurobj->m_iForeignFlags), pos.x, pos.y, pos.z);
						VALIDATOR_LOG(m_pLog, buf);
						if (m_vars.bBreakOnValidation)
							DoBreak
									pobj->m_ig[0]
											.x = pobj->m_ig[1].x = pobj->m_ig[0].y = pobj->m_ig[1].y = GRID_REG_PENDING;
						flags |= 8;
						goto skiprepos;
					}
				}
				...
				for (ix = igx[0]; ix <= igx[1]; ix++)
					for (iy = igy[0]; iy <= igy[1]; iy++)
					{
						if ((flags & 4) && (ix | iy | pgrid->size.x - 1 - ix | pgrid->size.y - 1 - iy) >= 0)
						{
							...
							AABB bbox;
							pgrid->BBoxFromGrid(gscale * Vec3(ix, iy, igz[0]), gscale * Vec3(ix + 1, iy + 1, igz[1]), (Vec3 *)&bbox);
			...
			else
			{
				...
				if (CPhysicalPlaceholder *portal = pent->GetLastPortal())
				{
					...
					if (!(insideWhat->GetStatus(&scp) ^ -(portal->m_iForeignFlags & TRIGGER_PORTAL_INV) >> 31 & 1))
					{
						...
						pobj->SetParams(&pp);
						pBBox[0] = pent->m_BBox[0];
						pBBox[1] = pent->m_BBox[1];
					}
				}
	...
	if (bBBoxUpdated | bSimClassUpdated)
	{
		...
		if (pent->m_flags & (pef_monitor_state_changes | pef_log_state_changes))
		{
			...
			if (bBBoxUpdated && ((INT_PTR)m_pEventClients[EventPhysBBoxChange::id][0] | (INT_PTR)m_pEventClients[EventPhysBBoxChange::id][1]) && !(m_vars.lastTimeStep * pent->GetMassInv()))
			{
				...
				eventBB.pEntity = pent;
				eventBB.pForeignData = pent->m_pForeignData;
				eventBB.iForeignData = pent->m_iForeignData;
...
void CPhysicalWorld::SyncPortal(CPhysicalPlaceholder *portal)
{
	...
	{
		pe_status_pos sp;
		sp.pGridRefEnt = portal->m_pEntBuddy;
		portal->GetStatus(&sp);
		pe_params_pos pp;
...
int CPhysicalWorld::DeformEntityPart(CPhysicalEntity *pent, int i, pe_explosion *pexpl, geom_world_data *gwd, geom_world_data *gwd1, int iSource)
{
	...
	epum.bInvalid = 0;
	if ((pent->m_parts[i].idmatBreakable >> 7 | pexpl->iholeType) != 0 && !(pent->m_parts[i].idmatBreakable & 128 << pexpl->iholeType) || pent->m_parts[i].pPhysGeomProxy->pGeom->GetiForeignData() == DATA_UNSCALED_GEOM)
		return 0;
	for (int j = 0; j < min(pent->m_nParts, 5); j++)
		if (i != j && pent->m_parts[j].flags & geom_log_interactions && pent->m_parts[j].pPhysGeom->pGeom->PointInsideStatus(((pexpl->epicenter - pent->m_pos) * pent->m_qrot - pent->m_parts[j].pos) * pent->m_parts[j].q * (pent->m_parts[j].scale == 1.0f ? 1.0f : 1.0f / pent->m_parts[j].scale)))
			return 0;
...
void CPhysicalWorld::SimulateExplosion(pe_explosion *pexpl, IPhysicalEntity **pSkipEnts, int nSkipEnts, int iTypes, int iCaller)
{
	...
	for (i = 0; i < nSkipEnts; i++)
		if (pSkipEnts[i] && GetGrid(pSkipEnts[i]) != GetGrid(pGridRef))
	...
	if (pexpl->nOccRes > 0 || pexpl->holeSize > 0)
		for (nents = GetEntitiesAround(pexpl->epicenter - Vec3(1, 1, 1) * pexpl->rmax, pexpl->epicenter + Vec3(1, 1, 1) * pexpl->rmax, pents,
																	 ent_terrain | ent_static | ent_rigid, pGridRef, 0, iCaller) -
								 1;
				 nents >= 0; nents--)
			if (pents[nents]->m_iSimClass < 1 || pents[nents]->GetMassInv() <= 0)
			{
				...
				QuatT trans = GridTrans(pGridRef, pents[nents]);
				{
					...
					for (i1 = bEntChanged = 0; i1 < pents[nents]->GetUsedPartsCount(iCaller); i1++)
						...
						{
							bBreak = (m_vars.breakImpulseScale || pents[nents]->m_flags & pef_override_impulse_scale || pexpl->forceDeformEntities) && iTypes & 1 << pents[nents]->m_iSimClass &&
								...
								!(pmd = (mesh_data *)pents[nents]->m_parts[i].pPhysGeomProxy->pGeom->GetData()) || pmd->nIslands <= 1)
						{
							...
							else
							{
								for (iprim = pmd->nTris - 1; iprim >= 0 &&
																						 !(m_SurfaceFlagsTable[epc.idmat[1] = pents[nents]->GetMatId(pmd->pMats[iprim], i)] & sf_manually_breakable);
										 iprim--)
									;
								epc.iPrim[1] = iprim;
							}
...
void CPhysicalWorld::DrawPhysicsHelperInformation(IPhysRenderer *pRenderer, int iCaller)
{
	...
	if (m_vars.iDrawHelpers & 0x100 << 6)
		for (SEntityGrid *pgrid = m_entgrid.m_next; pgrid; pgrid = pgrid->m_next)
			for (int i = 0; i < 2; i++)
				for (int ix = 0; ix <= pgrid->size[i]; ix++)
				{
	...
	if (m_vars.bLogActiveObjects)
	{
		...
		if (m_vars.bLogActiveObjects & 2)
			for (pent = m_pTypedEntsPerm[4]; pent; pent = pent->m_next)
				if (!(pent->m_flags & pef_disabled) && pent->IsAwake())
				{
					++nCount;
...
void CPhysicalWorld::AddEntityProfileInfo(CPhysicalEntity *pent, int nTicks)
{
	...
	if (i == m_nProfiledEnts)
	{
		...
		ppi.pName = m_pRenderer ? m_pRenderer->GetForeignName(pent->m_pForeignData,
																													pent->m_iForeignData, pent->m_iForeignFlags)
														: "noname";
	}
	else
	{
	...
	for (pEvent = pEventFirst; pEvent; pEvent = pEvent->next)
					...
					pEvent->idval > EventPhysCollision::id && ((CPhysicalEntity *)((EventPhysMono *)pEvent)->pEntity)->m_iDeletionTime))
		{
			if (pEvent->idval == EventPhysPostStep::id)
			{
				...
				if (pent->m_nSyncColliders)
					for (int i = 0; i < pent->m_nColliders; i++)
			...
			if (bRWIorPWI && ((EventPhysRWIResult *)pEvent)->OnEvent)
				...
				for (pClient = m_pEventClients[pEvent->idval][1]; pClient; pClient = pClientNext)
				{
...
int CPhysicalEntity::SetStateFromTypedSnapshot(TSerialize ser, int iSnapshotType, int flags)
{
	...
	return g_pTypedEnts[min((int)(CRY_ARRAY_COUNT(g_pTypedEnts)), max(0, iSnapshotType))]->SetStateFromSnapshot(ser, flags);
}
...
void CPhysicalWorld::SerializeGarbageTypedSnapshot(TSerialize ser, int iSnapshotType, int flags)
{
	...
	g_pTypedEnts[min((int)(CRY_ARRAY_COUNT(g_pTypedEnts)), max(0, iSnapshotType))]->GetStateSnapshot(ser, flags);
}
...
inline linkedpt *unlink(linkedpt *p)
{
	p->next[0]->next[1] = p->next[1];
