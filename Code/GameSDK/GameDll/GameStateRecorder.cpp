							...
							if (pItem)
							{
								...
								if (pWeapon)
								{
									...
									if (pAmmoClass)
									{
										if (m_bLogWarning)
										{
											int curAmmoCount = (bAmmoMag ? pInventory->GetAmmoCount(pAmmoClass) : pWeapon->GetAmmoCount(pAmmoClass));
		...
		if (pActor)
		{
			...
			if (m_itSingleActorGameState != m_GameStates.end() && pActor->GetEntity() && m_itSingleActorGameState->first == pActor->GetEntity()->GetId())
			{
				...
				if (pVar)
				{
					...
					if (demo_forceGameState)
					{
						IRenderAuxText::Draw2dLabel(1, y + retY, 1.3f, fColor, false, demo_forceGameState == 1 ? " Override mode = (health)" : " Override mode = (all)");
						retY += 15;
					}
				}
